...
const Item = ({ item, type = "full", currencyCode }: ItemProps) => {
  ...
  return (
    <tr
      className={clx(
        "w-full bg-white py-4 md:py-7 border-b border-gray-100 last:border-b-0",
        type === "preview" && "px-4"
      )}
    ...
    >
      ...
      <td className="align-top w-24 md:w-28 pr-4">
        <LocalizedClientLink
          href={`/products/${item.product_handle}`}
          className="relative block w-24 md:w-28 h-24 md:h-28 rounded-md overflow-hidden bg-gray-200 cursor-pointer"
        >
          <img
          ...
          />
          <div className="absolute top-0 left-0 h-full w-full bg-black bg-opacity-30 md:bg-opacity-0 flex justify-center items-center transition duration-200 ease-in-out md:group-hover:bg-opacity-30">
            <DeleteButton
              id={item.id}
